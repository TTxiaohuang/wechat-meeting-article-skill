...
def render_literature(data: dict[str, Any], index: int, branded: bool = False) -> str:
    ...
    for paper_index, paper in enumerate(data.get("papers") or [], start=1):
        ...
        # ── Summary card: metadata + key findings ──
        findings = paper.get("findings") or []
        findings_html = ""
        if findings:
            items = "".join(
                f'<li style="margin:5px 0;color:{TEXT};font-size:14px;line-height:1.75;">{esc(f)}</li>'
                for f in findings
            )
            findings_html = (
                f'<p style="margin:12px 0 0;color:{ACCENT_2};font-size:14px;font-weight:700;line-height:1.5;">核心发现</p>'
                f'<ul style="margin:5px 0 0;padding-left:16px;">{items}</ul>'
            )
        parts.append(
            f'<section style="margin:16px 0 8px;padding:16px 15px;border-radius:8px;'
            f'border:1px solid {BORDER};background:{WARM};">'
            f'<p style="margin:0 0 4px;color:{MUTED};font-size:12px;font-weight:700;line-height:1.5;">文献 {paper_index}</p>'
            f'<p style="margin:0;color:{TEXT};font-size:18px;font-weight:800;line-height:1.45;">{esc(title)}</p>'
        ...
        for field_label, field_key in [
            ...
            ("方法与数据", "methods_data"),
            ("讨论价值", "discussion_value"),
        ]:
            text = paper.get(field_key) or ""
            if text:
                parts.append(
                    f'<section style="margin:10px 0 0;padding:10px 14px;border-left:3px solid {BORDER};">'
                    f'<p style="margin:0;color:{MUTED};font-size:12px;font-weight:700;line-height:1.5;text-transform:uppercase;">{esc(field_label)}</p>'
                    f'{paragraphs(text, size=14, margin_top=3)}'
                    "</section>"
                )
        ...
        # ── Discussion comments as horizontal scroll cards ──
        comments = paper.get("comments") or []
        if comments:
            parts.append(
                f'<p style="margin:14px 0 0;color:{MUTED};font-size:13px;line-height:1.6;">'
                f'左右滑动查看讨论摘录</p>'
            )
            comment_cards = []
            comment_total = len(comments)
            for ci, comment in enumerate(comments, start=1):
                speaker = comment.get("speaker") or "同学"
                comment_cards.append(
                    f'<section style="box-sizing:border-box;display:inline-block;vertical-align:top;width:92%;'
                    f'max-width:340px;min-height:180px;margin:8px 12px 8px 0;padding:16px 16px 14px;white-space:normal;'
                    f'border:1px solid {BORDER};border-radius:8px;background:#ffffff;">'
                    f'<p style="margin:0;color:{MUTED};font-size:12px;line-height:1.4;text-align:right;">{ci}/{comment_total}</p>'
                    f'<p style="margin:0;color:{ACCENT};font-size:16px;font-weight:800;line-height:1.4;">{esc(speaker)}</p>'
                    f'{paragraphs(comment.get("text") or "", size=14, margin_top=8)}'
                    "</section>"
                )
            parts.append(
                '<section style="box-sizing:border-box;overflow-x:auto;white-space:nowrap;'
...
def render_free_discussion(data: dict[str, Any], index: int, branded: bool = False) -> str:
    ...
    parts.append(paragraphs(data.get("intro") or data.get("summary") or ""))
    parts.append(render_section_images(data.get("images") or []))
    ...
    if items:
        ...
        for ci, item in enumerate(items, start=1):
            speaker = item.get("speaker") or "参会者"
            cards.append(
                f'<section style="box-sizing:border-box;display:inline-block;vertical-align:top;width:92%;'
                f'max-width:340px;min-height:180px;margin:8px 12px 8px 0;padding:16px 16px 14px;white-space:normal;'
                f'border:1px solid {BORDER};border-radius:8px;background:#ffffff;">'
                f'<p style="margin:0;color:{MUTED};font-size:12px;line-height:1.4;text-align:right;">{ci}/{total}</p>'
                f'<p style="margin:0;color:{ACCENT};font-size:16px;font-weight:800;line-height:1.4;">{esc(speaker)}</p>'
                f'{paragraphs(item.get("text") or "", size=14, margin_top=8)}'
                "</section>"
            )
        parts.append(
            '<section style="box-sizing:border-box;overflow-x:auto;white-space:nowrap;'
