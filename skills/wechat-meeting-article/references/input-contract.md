...
Create `article.json` with UTF-8 encoding. Required top-level keys are `meta` and `sections`; recommended top-level keys are `theme`, `template`, `palette`, and optionally `assets`.
...
- Set `"template"` to `"classic"` for the default concise intro layout, `"notebook"` for a reading-note feel, `"briefing"` for a report-like rhythm, or `"fieldnote"` for a light field-note style.
- Set `"palette"` to `"classic"`, `"forest"`, `"blueprint"`, or `"warm"`. Use only one palette per article unless you are manually revising the renderer.
