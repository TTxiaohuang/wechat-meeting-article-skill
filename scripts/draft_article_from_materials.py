...
def build_article(input_dir: Path) -> dict[str, Any]:
    ...
    return {
        "_meta": {
            ...
            "warning": (
                "This is a scaffold generated by deterministic heuristics. It must be expanded "
                "with AI/source-grounded writing before publication and should not be delivered directly."
            ),
            "intake_gate": {
                "material_folder": {"value": str(input_dir), "status": "inferred_from_input"},
                "date": {"value": "", "status": ""},
                "editor": {"value": "", "status": ""},
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("article.scaffold.json"))
